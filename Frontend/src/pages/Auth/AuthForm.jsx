...
export default function AuthForm({setAuthForm,setWhichForm,whichForm}) {
  ...
  return (
    <div className="h-screen  z-100  w-full fixed left-1/2  -translate-x-1/2 top-1/2 -translate-y-1/2 backdrop-blur-sm  flex items-center justify-center p-4 max-sm:p-0">
      ...
      className="relative flex  rounded-4xl  items-center justify-center w-full max-w-6xl h-full ">
        <button onClick={()=>setAuthForm(false)} className="cursor-pointer absolute  z-20 right-3 top-3 text-white/70 text-4xl ">
            <IoClose />
        </button>
  ...
  <h2 className="text-6xl font-semibold text-red-100 font-['inter'] leading-15 tracking-wide mb-10 max-md:text-3xl max-md:leading-10  max-md:tracking-normal">
    Sign up to start <br className='max-md:hidden'/> meeting for free
  </h2>
